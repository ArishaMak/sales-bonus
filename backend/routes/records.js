...
router.get("/", async (req, res) => {
  try {
    ...
    if (q) {
      ...
      where.push(`(
        pr.purchase_id LIKE ? 
        OR pr.total_amount LIKE ? 
        OR CONCAT(c.first_name,' ',c.last_name) LIKE ?
        OR CONCAT(s.first_name,' ',s.last_name) LIKE ?
        OR pr.seller_id LIKE ?
      )`);
    ...
    if (sku) {
      where.push(`
        EXISTS(
          ...
          WHERE pi.purchase_id = pr.purchase_id 
          AND pi.sku = ?
        )
      `);
      params.push(sku);
    ...
    const data = rows.map((r) => ({
      ...
      customer_name: `${r.customer_first || ""} ${
        r.customer_last || ""
      }`.trim(),
    ...
    res
      .status(500)
      .json({ error: "Ошибка сервера при получении записей" });
  }
});
